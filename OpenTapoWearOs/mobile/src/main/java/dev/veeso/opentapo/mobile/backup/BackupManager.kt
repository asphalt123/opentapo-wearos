package dev.veeso.opentapo.mobile.backup

import android.content.Context
import android.util.Log
import dev.veeso.opentapo.mobile.MainActivity
import dev.veeso.opentapo.mobile.account.AccountCrypto
import dev.veeso.opentapo.mobile.account.AccountStore
import dev.veeso.opentapo.mobile.timer.TimerStore
...
object BackupManager {
    ...
    fun restore(context: Context, backup: FullBackup, passphrase: String) {
        ...
        // 4. ré-arme tout
        try {
            dev.veeso.opentapo.mobile.timer.TimerManager.rescheduleAll(context)
            dev.veeso.opentapo.mobile.geofence.GeofenceManager.refresh(context)
            dev.veeso.opentapo.mobile.monitor.DeviceMonitorScheduler.schedule(context)
            dev.veeso.opentapo.mobile.monitor.DeviceMonitorReceiver.refreshPersistent(context)
            dev.veeso.opentapo.mobile.voice.VoiceShortcutManager.refresh(context)
            dev.veeso.opentapo.mobile.SyncHelper.sendActiveAccountToWear(context)
            val creds = AccountStore.activeCredentials(context)
            if (creds != null && backup.devicesJson.isNotEmpty()) {
                dev.veeso.opentapo.mobile.SyncHelper.sendDevicesToWear(context, backup.devicesJson)
            }
        } catch (e: Exception) {
