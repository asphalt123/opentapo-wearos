package dev.veeso.opentapo.mobile.energy
...
import dev.veeso.opentapo.mobile.data.CredentialsProvider
import dev.veeso.opentapo.mobile.data.DeviceStore
 ...
 *
 * Opened from [dev.veeso.opentapo.mobile.control.DeviceControlActivity] via
...
class EnergyActivity : AppCompatActivity() {
    ...
    companion object {
        ...
        const val ACTION_VIEW = "dev.veeso.opentapo.mobile.energy.VIEW"
        ...
        suspend fun query(context: Context, deviceId: String): EnergyQuery {
            ...
            return try {
                val client = try {
                    ...
                    dev.veeso.opentapo.mobile.tapo.api.tapo.TapoClient(
                        Inet4Address.getByAddress(bytes) as Inet4Address
                    )
                } catch (_: Exception) {
                    dev.veeso.opentapo.mobile.tapo.api.tapo.TapoClient(device.endpoint)
                }
