package dev.veeso.opentapowearos.complication
 ...
 * complication-config extras) or directly from [dev.veeso.opentapowearos.MainActivity].
...
class ComplicationConfigActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(dev.veeso.opentapowearos.R.layout.activity_complication_config)

        val box: LinearLayout = findViewById(dev.veeso.opentapowearos.R.id.complication_device_box)
        val empty: TextView = findViewById(dev.veeso.opentapowearos.R.id.complication_empty)
        val devices = ComplicationHelper.allCached(this)
        val selected = ComplicationHelper.selectedDeviceId(this)

        if (devices.isEmpty()) {
            empty.visibility = View.VISIBLE
            empty.text = getString(dev.veeso.opentapowearos.R.string.complication_no_device)
        } else {
